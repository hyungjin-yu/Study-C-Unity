...
## 제네릭 오브젝트 풀 (T where MonoBehaviour)

[[Unity - Object Pooling과 메모리 관리]]의 풀은 `GameObject` 하나로만 관리해서 매번 `GetComponent`로 캐스팅해야 함. 타입을 제네릭으로 받으면 캐스팅 없이 바로 쓸 수 있음:

```csharp
public class ObjectPool<T> where T : MonoBehaviour
{
    private Queue<T> pool = new Queue<T>();
    public T Get() => pool.Count > 0 ? pool.Dequeue() : null;
